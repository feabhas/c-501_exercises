...
// Function-like macro to allow output via the
// seven-segment display or USART.
// To switch, set the STEP_USE_USART macro
// in the build configuration.
//
#ifdef STEP_USE_USART
#include "usart.h"
#define DISPLAY(step)   do {                           \
                          usart_send_string("Step: "); \
                          usart_send(step + '0');      \
                          usart_send_string("\n\r");   \
                        } while(0)
#define DISPLAY_INIT    usart_init

#else
#include "sevensegment.h"
#define DISPLAY(step)   sevensegment_display(step);
#define DISPLAY_INIT    sevensegment_init
#endif
